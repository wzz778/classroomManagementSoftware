  ...
  <div class="mainBox" >
    ...
    >
      <el-form-item label="新密码" style="margin-bottom=0">
        <el-input
          placeholder="请输入新密码"
          v-model="forgetPasswordInfo.newPassword"
          show-password
      ...
      <el-form-item label="确认新密码" style="margin-bottom=0">
        <el-input
          placeholder="请确认新密码"
          v-model="forgetPasswordInfo.surePassword"
          show-password
      ...
      <el-button class="forgetPasswordBtn" type="primary" @click="updatePasswordFun">修改</el-button>
    </el-form>
  </div>
</template>
...
import {getCode,updatePassword} from '@/api/student/yxyAxios'
export default {
  name: "ForgetPassword",
  data() {
    return {
  ...
  methods: {
    judgePassword() {
      ...
      let judgeRes = passwordReg.test(this.forgetPasswordInfo.newPassword);
      if (judgeRes == false) {
        this.passwordStyle = {
          height: "25px",
        };
    ...
    judgeSurePassword() {
      console.log(11);
      console.log(this.forgetPasswordInfo.newPassword);
      console.log(this.surePassword);
      if (
        this.surePassword !=
        this.forgetPasswordInfo.newPassword
      ) {
    ...
    judgeEmail() {
      ...
      console.log(111);
      if (EmailReg.test(this.forgetPasswordInfo.email) == false) {
        this.emailStyle = {
          height: "25px",
        };
    ...
    getCode() {
      if (this.forgetPasswordInfo.email == "") {
        Message.warning("请填写邮箱！");
      } else if (this.judgeEmailRes == false) {
        Message.warning("邮箱格式不合规范，请重新填写！");
      } else {
        let data = {
          email: this.forgetPasswordInfo.email,
        };
    ...
    updatePasswordFun() {
      if (this.judgePasswordRes == false) {
        ...
        let data=this.forgetPasswordInfo
